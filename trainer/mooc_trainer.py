...
class DatasetMooc(Dataset):
    def __init__(self, dataset, indicator, word_dict):
        super(DatasetMooc, self).__init__()
        self.texts = None
        self.aspects = None
        self.label = None
        self.word_dict = word_dict
        if indicator == 'train':
            mooc_df = pd.read_csv('output/mooc_data_train.csv')
            self.texts = mooc_df['text']
            self.aspects = mooc_df['term']
            self.label = mooc_df['polarity'].map(
                {'好评': 1, '中评': 0, '差评': -1})
        elif indicator == 'test':
            mooc_df = pd.read_csv('output/mooc_data_test.csv')
            self.texts = mooc_df['text']
            self.aspects = mooc_df['term']
            self.label = mooc_df['polarity'].map(
                {'好评': 1, '中评': 0, '差评': -1})
...
class MoocTrainer(object):
    ...
    def encoder(self, dataset):
        texts = None
        df = pd.read_csv('output/mooc_data.csv')
        texts = df['text']
        filtered_texts = texts.apply(preprocessing_mooc)
        word_dict = dict()
        word_dict['NAN'] = 0
        couter = 1
        word_list = []
    ...
    def load_data(self, dataset):
        ...
        train_dataset = DatasetMooc(dataset, "train", self.word_dict)
        test_dataset = DatasetMooc(dataset, "test", self.word_dict)
